        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {steps.map((step, index) => {
            ...
            return (
              <Card 
                key={index} 
                className="bg-gray-900 border-gray-800 p-8 text-center relative overflow-hidden hover:scale-105 transition-all duration-500 group animate-fade-in"
                style={{ animationDelay: `${index * 0.2}s` }}
              >
                {/* Step Number */}
                <div className="absolute top-4 right-4 text-6xl font-bold text-gray-700 transition-all duration-300 group-hover:text-yellow-400/20">
                  {step.number}
                </div>
                
                {/* Icon */}
                <div className="inline-flex p-4 rounded-full bg-gradient-to-r from-yellow-400 to-orange-500 mb-6 transition-all duration-300 group-hover:scale-110">
                ...
                <p className="text-gray-400 text-lg leading-relaxed transition-all duration-300 group-hover:text-gray-300">
                  {step.description}
                </p>
